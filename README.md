...
| <a name="input_build_job_empty_dirs"></a> [build\_job\_empty\_dirs](#input\_build\_job\_empty\_dirs) | A map of maps of name:{name, mount\_path, medium} for which each named value will result in a named empty\_dir mounted with. | <pre>map(map(object({<br>    name : string<br>    mount_path : string<br>    medium : string<br>  })))</pre> | `{}` | no |
